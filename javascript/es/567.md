...
- async函数的特点
    ...
    - async 函数返回的总是 `Promise`对象，可以用 `then()` 方法执行下一步操作
    - 对 Generator函数声明的部分替代
        - 使用 `async` 替代 `function*` 的 `*`
        - 使用 `await` 替代 `yield`

- await 的返回值
    - 如果函数是普通函数，则函数的返回值，就是 await的返回值
    - 如果是 Promise 对象，如果调用 `resolve()`、`reject()` 时传递的参数就是 await的参数，如果没有传参，await 是 undefined
...
- 执行顺序
    - 执行初始化代码时，所有代码按照调用顺序执行，包括 async函数
    - 执行 async 函数时，按照初始化代码开始执行，遇到 await 则将任务发送到 事件管理模块，然后跳出当前函数，继续执行后续的初始化代码
    - 当初始化代码执行结束，事件循环开始执行 await 的任务
