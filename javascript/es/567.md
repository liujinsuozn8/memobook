...
- 使用严格模式后，**语法和行为的改变**
    ...
    5. 函数的参数名，不能同名
        ...
        ```
    5. 不能使用 `with` 语句
    6. js的保留字、eval、arguments 不能被重写
    7. 不能用 `delete` 删除显示声明的标识符、函数、或具名函数
    8. 不能访问 `arguments.callee`
    9. 不能访问 `函数.caller`
...
- 创建Promise对象
    ...
    - 传入的回调函数
        ...
        - <label style='color:red'>函数的执行是同步的，不会作为回调函数使用</label>
        - 函数内部的异步操作，仍然是异步的
...
- 解构嵌套的异步调用，并处理异常
    ...
    - <label style='color:red'>如何完成链式调用？</label>
...
- 执行顺序
    ...
    4. 执行 await函数中的异步任务时，将任务发送到 事件管理模块，然后<label style="color:red">跳出当前await函数和async函数</label>，继续执行后续的初始化代码
