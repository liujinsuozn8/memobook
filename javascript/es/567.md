...
- 使用严格模式后，**语法和行为的改变**
    ...
    8. 不能访问 `函数.caller`
    9. 不能使用 `delete` 删除显示声明的标识符、名称、具名函数
...
## Promise
[top](#catalog)
- 参考: [promise](/javascript/underlyingPrinciple/index.md)
...
- Generator函数的特点
    ...
    - 通过 `next()` 方法执行
    - 遇到 `yield` 表达式时，执行表达式，并将结果作为 `next()` 方法的返回结果，然后**暂停函数**
...
- 示例
    - 参考代码
        - [src/es6/generator/base.js](src/es6/generator/base.js)
