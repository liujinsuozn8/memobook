...
2. [语句与代码分块](#语句与代码分块) 形成了  [组织代码分块的方式](#组织代码分块的方式)，也造成了:
    - [词法作用域--模块化层次的主要体现之一](#词法作用域--模块化层次的主要体现之一)
    - [变量作用域--模块化层次的主要体现之一](#变量作用域--模块化层次的主要体现之一)
...
- 两种顶层声明
    ...
    - 运行时，每个块按照相同的规则变量两类声明，来获得词法声明列表与变量声明列表

## 提升
### 提升的本质
[top](#catalog)
- 什么是变量提升
    - 在当前作用域的任意位置，用var声明的变量，其标识符会被提升到当前作用域或其外层的、最低级别的`变量作用域`的开始处
