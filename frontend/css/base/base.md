...
- 如果同时对多个元素使用`float:right`，元素会**逆序排列**
...
- 通过`margin:auto`、`top:0`、`bottom:0`、`left:0`、`right:0`，可以使元素在包含块中水平垂直居中
    - 如果当前元素大小超过了的定位元素的大小，则设置无效，显示的结果是**左对齐**
        - 这种情况需要手动调整位置
...
- 元素层级的特点
    ...
    - <label stlye='color:red'>元素遮盖的前提，当前元素有自己的颜色</label>
        - 因为颜色没有继承，所以如果不设置颜色，即颜色为透明，即使发生遮盖，也会正常显示
...
- 示例
    ...
    - 全部开启绝对定位,并分别设置层级,层级越大越优先显示
        ...
        - html
            ...
            ```
        - 页面结果
            - ![zindex_02.png](imgs/position/zindex/zindex_02.png)
